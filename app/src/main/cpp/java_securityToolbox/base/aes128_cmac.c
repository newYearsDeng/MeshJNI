...
void LoadMacKey(uint8_t *key)
{
	...
	uint8_t plain[16] = { 0x00, 0x00, 0x00, 0x00,
						  ...
						  0x00, 0x00, 0x00, 0x00,
						  0x00, 0x00, 0x00, 0x00};
	uint8_t Rb[16] = { 	0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x00, 0x87 };
	uint8_t c0[16];
	...
	aes128_encryption(MACkey, plain, c0);
...
void GenerateMAC(int len, uint8_t *add, uint8_t *macvalue)
{
	...
	if (len % 16 == 0 && len!=0) {
		...
		for (i = 0; i < block-1; i++)
		{
			...
			aes128_encryption(MACkey, Blocktemp, IVtemp);
		}

		ArrayXor(16, &add[(block-1)*16], IVtemp, Blocktemp);
		ArrayXor(16, Blocktemp, k1, Blocktemp);
		aes128_encryption(MACkey, Blocktemp, macvalue);
