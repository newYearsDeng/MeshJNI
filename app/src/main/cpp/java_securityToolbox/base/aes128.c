...
static void key_expansion(const unsigned char *key, unsigned char *w);
 ...
 */
unsigned char R[4] = {0x02, 0x00, 0x00, 0x00};
...
int aes128_encryption(const uint8_t key[], const uint8_t plaintext[], uint8_t* ciphertext)
{
    uint8_t w[176] ;
    key_expansion(key, w);
    cipher((uint8_t *)plaintext, ciphertext, w);
    return 0;
}

int aes128_decryption(const uint8_t key[], const uint8_t cypher[], uint8_t* plain_text)
{
    uint8_t w[176] ;
    key_expansion(key, w);
    inv_cipher((uint8_t *)cypher, plain_text, w);
    return 0;
}
